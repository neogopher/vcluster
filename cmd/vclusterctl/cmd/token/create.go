...
func CreateBootstrapToken(ctx context.Context, vClient *kubernetes.Clientset, expires string, controlPlane bool) (platformEndpoint, apiEndpoint, token, tokenSecretName, caHash string, err error) {
	...
	if err != nil {
		...
		caHash = pubkeypin.Hash(caCerts[0])
	}
...
func detectPlatformConnectionFromKubeconfig(globalFlags *flags.GlobalFlags) (project, vclusterName string, err error) {
	...
	if name, project, ok := parseVirtualClusterInstanceContext(currentContext); ok {
...
// parseVirtualClusterInstanceContext parses contexts named
// vcluster-platform-vcluster_{instance}_{project}, matching
// kubeconfig.VirtualClusterInstanceContextName encoding.
func parseVirtualClusterInstanceContext(contextName string) (vclusterName, project string, ok bool) {
